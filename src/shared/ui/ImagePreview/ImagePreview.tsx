...
const ImagePreview = ({ editing, ...props }: Props) => {
    ...
    const nextPos = useCallback(() => {
        setPosition(prev =>
            props.imagesSrc && prev < props.imagesSrc.length - 1
                ? prev + 1
                : prev
        )
    }, [])
            ...
            {editing && (
                <InlineWrapper className={style.bar}>
                    {editing.onDownload && (
                        <h2>
                            <HiDownload
                                onClick={() => editing.onDownload?.(position)}
                            />
                        </h2>
                    )}
                    {editing.onDelete && (
                        <HiTrash onClick={() => editing.onDelete?.(position)} />
                    )}
                    {editing.onEdit && (
                        <HiPencil onClick={() => editing.onEdit?.(position)} />
                    )}
                </InlineWrapper>
            )}
            <AnimatePresence>
